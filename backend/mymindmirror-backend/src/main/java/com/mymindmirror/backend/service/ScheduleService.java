...
public class ScheduleService {
    ...
    // ScheduleService.java (only relevant parts shown – keep existing code but add/override as below)
    ...
    private List<Map<String, Object>> simpleSchedule(List<ScheduleTaskRequest.TaskItem> tasks,
        ...
        // Track where we currently are in the calendar
        ...
        for (ScheduleTaskRequest.TaskItem task : tasks) {
            ...
            // Get estimated hours (default to 1.0 hour if missing)
            ...
            while (currentDayOffset < maxDays && !scheduled) {
                ...
                // If no slots left today, advance to the next day
                ...
                // Initialize the clock to the start of the current slot
                if (currentTimeInSlot == null) {
                    currentTimeInSlot = currentSlot.getStart();
                }

                // Calculate when this task would finish
                LocalTime proposedEnd = currentTimeInSlot.plusMinutes(estMinutes);

                // If the task overflows the current slot (or passes midnight), move to the next slot
                if (proposedEnd.isBefore(currentTimeInSlot) || proposedEnd.isAfter(currentSlot.getEnd())) {
                    currentSlotIndex++;
                    currentTimeInSlot = null;
                    continue;
                }

                // It fits! Add it to the schedule
    ...
    private List<ScheduleTaskRequest.TaskItem> collectUnscheduledTasks(User user) {
        ...
        for (RoadmapTask rt : roadmapTasks) {
            ...
            item.setEstimatedHours(estimateDuration(rt.getDetails()));
        ...
        for (Task mt : milestoneTasks) {
            ...
            item.setEstimatedHours(estimateDuration(mt.getDetails()));
