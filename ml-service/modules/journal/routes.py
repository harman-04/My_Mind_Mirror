...
def get_gemini_journal_analysis(journal_text, api_key=None):
    """
    ...
    **LANGUAGE & STYLE INSTRUCTION:**
    ...
    try:
        ...
        if full_analysis is None:
            ...
            return {
                ...
                "keyPhrases": [],
                "warning": "AI analysis incomplete due to service issue."
            }

    except GeminiAPIException as e:
        logger.error(f"Gemini API error in journal analysis: {e}")
        return {
            "emotions": {},
            "coreConcerns": [],
            "summary": "AI analysis temporarily unavailable. Please try again later.",
            "growthTips": ["Check your Gemini API key and quota.", "Ensure the ML service is running."],
            "keyPhrases": [],
            "error": {"message": str(e), "code": getattr(e, 'status_code', 500)},
            "warning": "AI analysis incomplete due to service issue."
        }
    except Exception as e:
        logger.error(f"Unexpected error in journal analysis: {e}", exc_info=True)
        return {
            "emotions": {},
            "coreConcerns": [],
            "summary": "An unexpected error occurred during analysis.",
            "growthTips": ["Please try again later. If the problem persists, contact support."],
            "keyPhrases": [],
    ...
    if total_score > 0 and abs(total_score - 1.0) > 0.01:
        ...
        full_analysis["emotions"] = processed_emotions

    # Ensure other fields are lists/strings
...
def analyze_journal():
    ...
    # Call the unified function, passing the key
    ai_analysis_results = get_gemini_journal_analysis(journal_text, api_key=api_key)

    # Check if there was an error
    if "error" in ai_analysis_results:
        logger.warning(f"Returning fallback analysis due to Gemini error: {ai_analysis_results['error']}")
    ...
    # Only add warning if it exists and is not None
    warning_val = ai_analysis_results.get("warning")
    if warning_val is not None:
        response_data["warning"] = warning_val
    ...
    emotion_weights = {
        ...
        'stress': -0.7, 'frustration': -0.5, 'gratitude': 0.9, 'hope': 0.8
    }
    ...
    logger.info(f"Journal analysis completed. Mood Score: {response_data['moodScore']:.2f}")
...
def detect_anomalies(daily_data_list):
    ...
    # Anomaly thresholds
    mood_threshold_std = 0.8
    words_threshold_std = 1.0

    anomalies = []
    start_idx = ewma_span - 1 if len(df) >= ewma_span else 0
    ...
    for i in range(start_idx, len(df)):
        ...
        # Check mood anomaly
        if current_day['mood_ewma_std'] > 0:
            ...
            if abs(z_score_mood) > mood_threshold_std:
                ...
                mood_deviation_msg = (
                    ...
                    f"than your recent typical mood ({current_day['mood_ewma_mean']:.2f})."
                )
        elif current_day['averageMood'] != current_day['mood_ewma_mean']:
            is_mood_anomaly = True
        ...
        # Check words anomaly
        if current_day['words_ewma_std'] > 0:
            ...
            if abs(z_score_words) > words_threshold_std:
                ...
                words_deviation_msg = (
                    ...
                    f"than your recent typical word count ({current_day['words_ewma_mean']:.2f})."
                )
        elif current_day['totalWords'] != current_day['words_ewma_mean']:
            is_words_anomaly = True
        ...
        if is_mood_anomaly or is_words_anomaly:
            anomaly_details = {"date": current_date_str, "type": [], "message": ""}
            if is_mood_anomaly:
...
def train_and_save_clustering_model(user_id, journal_texts, n_clusters):
    ...
    try:
        ...
        logger.error(f"Invalid n_clusters value: {n_clusters}. Defaulting to 5.")
        n_clusters_int = 5

    actual_n_clusters = min(n_clusters_int, len(journal_texts))
    if actual_n_clusters <= 1:
        logger.warning(f"Not enough data for meaningful clusters for user {user_id}.")
        return None
...
def get_cluster_keywords_semantic(kmeans_model, journal_texts, num_keywords=5):
    ...
    for i, text in enumerate(journal_texts):
        if i < len(kmeans_model.labels_):
            ...
            logger.warning(f"Text index {i} out of bounds for kmeans_model.labels_.")
    ...
    for cluster_id, texts_in_cluster in clusters_data.items():
        ...
        try:
            ...
            if not non_empty_preprocessed_texts:
                logger.warning(f"Cluster {cluster_id+1} has no non-empty preprocessed texts.")
            ...
            if feature_names.size > 0:
                ...
                descriptive_keywords = [
                    ...
                    if tag.startswith('N') or tag.startswith('J') and word not in stop_words
                ][:num_keywords]
            ...
            logger.error(f"Error during keyword extraction for cluster {cluster_id+1}: {e}", exc_info=True)
...
def cluster_journal_entries_endpoint():
    ...
    if len(journal_texts) < 2:
        logger.warning(f"Not enough journal entries ({len(journal_texts)}) for clustering.")
    ...
    if actual_n_clusters <= 1:
        logger.warning(f"Adjusting n_clusters to {actual_n_clusters}.")
        response_data = {
            "numClusters": 0,
            "clusterThemes": {},
            "entryClusters": [],
    ...
    if sentence_model is None:
        logger.error("Semantic clustering model not loaded.")
    ...
    try:
        ...
        if kmeans_model is None:
            logger.error("train_and_save_clustering_model returned None.")
        ...
        if not cluster_themes or not response.get("clusterThemes"):
            logger.warning("get_cluster_keywords_semantic returned empty themes.")
            ...
            response["message"] = "Clustering successful, but specific themes could not be extracted."

        logger.info(f"Clustering completed for user {user_id}.")
