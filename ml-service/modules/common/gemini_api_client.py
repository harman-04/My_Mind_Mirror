...
class GeminiApiClient:
    ...
    def _make_api_call_with_retries(self, prompt_text, response_schema, temperature, timeout, api_key):
        ...
        # Use provided key if given, otherwise fallback to global
        key_to_use = api_key if api_key else Config.GEMINI_API_KEY

        if api_key:
            logger.info(f"Using user-provided API key (masked: {api_key[:4]}...{api_key[-4:]})")
        else:
            logger.info("Using global API key")

        if not key_to_use:
            logger.error("No Gemini API key available (neither per‑user nor global).")
            return None
        ...
        if response.status_code == 429:
            logger.warning("Gemini API returned 429 (RESOURCE_EXHAUSTED). Retrying...")
        ...
        if result.get("candidates") and result["candidates"][0].get("content") and result["candidates"][0]["content"].get("parts"):
            ...
            if result.get("promptFeedback") and result["promptFeedback"].get("blockReason"):
                ...
                logger.warning(f"Gemini API blocked response due to: {block_reason}")
                return None
            if response_schema:
                try:
                    ...
                    if json_match:
                        try:
                            ...
                            pass
                    logger.error("Gemini response did not contain valid JSON as expected.")
                    return None
            else:
