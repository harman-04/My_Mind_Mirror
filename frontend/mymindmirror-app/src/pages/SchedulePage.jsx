...
// 💡 NEW: Import all our abstracted queries and mutations
import {
...
// UX FIX: Distinct Slate/Gray color for Low Priority Work Tasks
...
const TaskDetailModal = ({ task, isOpen, onClose, onComplete, onEdit, onDelete, isCustomTask }) => {
  ...
  // 💡 Block Type Helper for Modals
  const getBlockTypeLabel = (type) => {
    switch(type) {
        case 'MEAL': return { icon: Utensils, label: 'Meal Time', color: 'text-orange-500' };
        case 'BREAK': return { icon: Coffee, label: 'Break', color: 'text-green-500' };
        case 'ROUTINE': return { icon: Activity, label: 'Routine/Habit', color: 'text-teal-500' };
        default: return { icon: Briefcase, label: 'Work Task', color: 'text-purple-500' };
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)' }}>
      ...
      <div className={`relative max-w-md w-full rounded-2xl ${bgClass} border ${borderClass} shadow-2xl overflow-hidden transform transition-all duration-300 scale-100 animate-in fade-in zoom-in`}>
        <div className="flex justify-between items-center p-5 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-purple-50 to-teal-50 dark:from-purple-900/30 dark:to-teal-900/30">
          <h3 className="text-xl font-poppins font-semibold bg-gradient-to-r from-purple-600 to-teal-600 dark:from-purple-400 dark:to-teal-400 bg-clip-text text-transparent">Schedule Details</h3>
          ...
          <div>
            <div className="flex justify-between items-start gap-2">
              <h4 className="text-lg font-semibold flex items-center gap-2">
                 <BlockIcon size={18} className={blockInfo.color} />
                 {task.title}
              </h4>
      ...
      <div className={`relative max-w-sm w-full rounded-2xl ${bgClass} border ${borderClass} shadow-2xl p-6 transform transition-all duration-300 animate-in fade-in zoom-in`}>
        <div className="flex flex-col items-center text-center">
          ...
          <div className="flex w-full gap-3">
            <button onClick={onClose} disabled={isLoading} className="flex-1 py-2.5 rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 font-medium hover:bg-gray-200 dark:hover:bg-gray-700 transition disabled:opacity-50">Cancel</button>
            <button onClick={onConfirm} disabled={isLoading} className="flex-1 py-2.5 rounded-xl bg-red-600 text-white font-medium hover:bg-red-700 transition flex items-center justify-center gap-2 disabled:opacity-70">
...
const MonthEvent = ({ event, onSelectEvent }) => {
  ...
  return (
    <button
      onClick={handleClick}
      className="w-full h-full text-left text-xs truncate text-white bg-transparent focus:outline-none flex items-center gap-1"
      title={event.title}
    >
      {isCompleted ? <span>✓</span> : (
          blockType === 'MEAL' ? <Utensils size={10} /> :
          blockType === 'BREAK' ? <Coffee size={10} /> :
          blockType === 'ROUTINE' ? <Activity size={10} /> : null
      )}
...
function SchedulePage() {
  ...
  // 💡 NEW: Inject Custom Hooks cleanly!
  ...
  const handleEventSelect = useCallback((event) => {
      ...
      setSelectedTask({
        ...
        blockType: event.resource?.blockType // 💡 Include blockType for modal
      });
    }, [customTasks]);
     ...
     <div className="relative w-full mb-8">
       <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-5 rounded-3xl border border-gray-200/60 dark:border-gray-700/60 bg-white/70 dark:bg-gray-900/60 backdrop-blur-xl p-5 sm:p-6 shadow-sm">
         ...
         <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3 w-full xl:w-auto">
           ...
           >
             {generateMutation.isPending ? <Loader size={18} className="animate-spin" /> : <RefreshCw size={18} className="transition-transform duration-300 group-hover:rotate-180" />}
             <span className="whitespace-nowrap">Generate</span>
           </button>
           {/* 💡 NEW: Re-optimise Today Button */}
                      <button
                        onClick={() => reoptimizeMutation.mutate()}
                        disabled={reoptimizeMutation.isPending || generateMutation.isPending}
                        className="group relative flex items-center justify-center gap-2 h-12 px-5 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-500 text-white font-medium shadow-md hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 disabled:opacity-50 disabled:hover:scale-100 w-full sm:w-auto"
                        title="Salvage the rest of today"
                      >
                        {reoptimizeMutation.isPending ? <Loader size={18} className="animate-spin" /> : <Zap size={18} className="text-white drop-shadow-sm" />}
           ...
           <div className="flex items-center p-1 rounded-2xl bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 w-full sm:w-auto">
             <button
               ...
               className={`flex-1 sm:flex-none px-4 h-10 rounded-xl text-sm font-medium transition-all duration-200 whitespace-nowrap ${
                 scheduleMode === "all" ? "bg-gradient-to-r from-purple-500 to-teal-500 text-white shadow-sm" : "text-gray-700 dark:text-gray-200 hover:bg-white/70 dark:hover:bg-gray-700"
               }`}
             >All Tasks</button>
             <button
               onClick={() => setScheduleMode("custom")}
               className={`flex-1 sm:flex-none px-4 h-10 rounded-xl text-sm font-medium transition-all duration-200 whitespace-nowrap ${
                 scheduleMode === "custom" ? "bg-gradient-to-r from-purple-500 to-teal-500 text-white shadow-sm" : "text-gray-700 dark:text-gray-200 hover:bg-white/70 dark:hover:bg-gray-700"
               }`}
             >Custom Tasks</button>
           </div>

           <button
             onClick={() => { setShowCustomForm(!showCustomForm); if (showCustomForm) setEditingCustomTask(null); }}
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 rounded-full p-1">
            {['ALL', 'PENDING', 'COMPLETED'].map(status => (
              <button key={status} onClick={() => setFilterCompleted(status)} className={`px-3 py-1 text-sm rounded-full transition ${filterCompleted === status ? 'bg-purple-500 text-white' : 'hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
          ...
          <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 rounded-full p-1">
            {['agenda', 'day', 'week', 'month'].map(view => (
              <button key={view} onClick={() => setCurrentView(view)} className={`px-3 py-1 text-sm rounded-full transition ${currentView === view ? 'bg-purple-500 text-white' : 'hover:bg-gray-200 dark:hover:bg-gray-700'}`}>
        ...
        <div className="mb-6 p-5 rounded-xl bg-gray-100/80 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 shadow-inner animate-in fade-in slide-in-from-top-2">
          ...
          <form onSubmit={editingCustomTask ? handleUpdateCustom : handleAddCustom} className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="flex justify-end gap-2 md:col-span-2">
               <button type="button" onClick={() => { setShowCustomForm(false); setEditingCustomTask(null); }} className="px-4 py-2 rounded-full bg-gray-300 dark:bg-gray-600 hover:bg-gray-400 transition">Cancel</button>
               <button type="submit" disabled={createCustomMutation.isPending || updateCustomMutation.isPending} className="flex items-center gap-2 px-5 py-2 rounded-full bg-purple-500 text-white hover:bg-purple-600 transition disabled:opacity-70 shadow-md">
                 {(createCustomMutation.isPending || updateCustomMutation.isPending) ? <Loader size={16} className="animate-spin" /> : editingCustomTask ? <Edit2 size={16} /> : <Plus size={16} />}
      ...
      <div className="h-[550px] md:h-[600px] mb-6 rounded-xl overflow-hidden shadow-inner custom-calendar relative">
        ...
        <DnDCalendar
          ...
          components={{
            ...
            agenda: {
              event: (props) => {
                ...
                // 💡 UPGRADED: Agenda Dot Colors based on Block Type
                let dotColor = isDarkMode ? '#6366f1' : '#a855f7'; // Purple default

                if (isCompleted) dotColor = '#22c55e'; // Green
                else if (blockType === 'MEAL') dotColor = isDarkMode ? '#ea580c' : '#f97316'; // Orange
                else if (blockType === 'BREAK') dotColor = isDarkMode ? '#16a34a' : '#22c55e'; // Green
                else if (blockType === 'ROUTINE') dotColor = isDarkMode ? '#0d9488' : '#14b8a6'; // Teal
                else if (priority === 'HIGH') dotColor = '#ef4444'; // Red
                else if (priority === 'LOW') dotColor = isDarkMode ? '#475569' : '#64748b'; // Slate
          ...
          eventPropGetter={(event) => {
            // 💡 UPGRADED: Calendar Block Colors based on Block Type
            let bgColor = isDarkMode ? '#6366f1' : '#a855f7';
            const blockType = event.resource?.blockType;

            if (event.resource?.completed) {
              bgColor = '#22c55e';
            } else if (blockType === 'MEAL') {
              bgColor = isDarkMode ? '#ea580c' : '#f97316'; // Orange
            } else if (blockType === 'BREAK') {
              bgColor = isDarkMode ? '#16a34a' : '#22c55e'; // Green
            } else if (blockType === 'ROUTINE') {
              bgColor = isDarkMode ? '#0d9488' : '#14b8a6'; // Teal
            } else {
              // Standard Work Tasks
              if (event.resource?.priority === 'HIGH') bgColor = '#ef4444';
              ...
              return (
                ...
                >
                  <div className="flex-1 flex items-center gap-3 flex-wrap">
                    <button onClick={() => handleToggleCompleteCustom(task)} disabled={updateCustomMutation.isPending} className={`focus:outline-none transition-opacity ${updateCustomMutation.isPending ? 'opacity-50 cursor-not-allowed' : ''}`}>
      ...
      <style>{`
        ...
        .custom-calendar { background: ${isDarkMode ? 'rgba(31, 41, 55, 0.4)' : 'rgba(255, 255, 255, 0.4)'}; border-radius: 1rem; }

        .rbc-toolbar {
          padding: 8px 12px; background: ${isDarkMode ? 'rgba(55, 65, 81, 0.3)' : 'rgba(243, 244, 246, 0.6)'};
          border-radius: 1rem 1rem 0 0; backdrop-filter: blur(8px); margin-bottom: 12px; flex-wrap: wrap; gap: 4px;
        }
        .rbc-toolbar button {
          background: ${isDarkMode ? '#374151' : '#E5E7EB'} !important; color: ${isDarkMode ? '#E5E7EB' : '#1F2937'} !important;
          border: none !important; border-radius: 9999px !important; padding: 4px 12px !important; font-size: 0.8rem; font-weight: 500; transition: all 0.2s;
        }
        .rbc-toolbar button:hover { background: ${isDarkMode ? '#4B5563' : '#D1D5DB'} !important; }
        .rbc-toolbar button.rbc-active { background: linear-gradient(135deg, #B399D4, #5CC8C2) !important; color: white !important; }

        .rbc-month-view, .rbc-week-view, .rbc-day-view { background: ${isDarkMode ? 'rgba(31, 41, 55, 0.5)' : 'rgba(255, 255, 255, 0.6)'}; backdrop-filter: blur(4px); border-radius: 0.75rem; overflow: hidden; border: none; }
        .rbc-header { padding: 8px 4px; font-weight: 600; background: ${isDarkMode ? '#1F2937' : '#F9FAFB'}; border-bottom: 1px solid ${isDarkMode ? '#374151' : '#E5E7EB'}; color: ${isDarkMode ? '#E5E7EB' : '#4B5563'}; }
        .rbc-time-gutter .rbc-timeslot-group { color: ${isDarkMode ? '#9CA3AF' : '#6B7280'}; font-size: 0.75rem; }
        .rbc-off-range-bg { background: ${isDarkMode ? 'rgba(55, 65, 81, 0.2)' : 'rgba(243, 244, 246, 0.4)'} !important; }
        .rbc-off-range { color: ${isDarkMode ? '#6B7280' : '#9CA3AF'} !important; }
        .rbc-day-bg:hover { background: ${isDarkMode ? 'rgba(99,102,241,0.05)' : 'rgba(168,85,247,0.05)'} !important; }
        .rbc-today-highlight { background: ${isDarkMode ? 'rgba(99,102,241,0.15)' : 'rgba(168,85,247,0.1)'} !important; }

        .rbc-show-more { color: ${isDarkMode ? '#A78BFA' : '#7C3AED'} !important; font-weight: 600; font-size: 0.75rem; padding: 2px 6px; border-radius: 4px; background: ${isDarkMode ? 'rgba(167,139,250,0.1)' : 'rgba(124,58,237,0.1)'}; display: inline-block; margin-top: 2px; cursor: pointer; transition: background 0.2s; }
        .rbc-show-more:hover { background: ${isDarkMode ? 'rgba(167,139,250,0.2)' : 'rgba(124,58,237,0.2)'}; }
        .rbc-overlay { background: ${isDarkMode ? '#1F2937' : '#FFFFFF'} !important; border-radius: 0.75rem !important; border: 1px solid ${isDarkMode ? '#374151' : '#E5E7EB'} !important; box-shadow: 0 10px 25px rgba(0,0,0,0.15) !important; z-index: 50 !important; max-height: 300px; padding: 0.5rem 0; overflow-y: auto !important; overflow-x: hidden !important; }
        .rbc-overlay::-webkit-scrollbar { width: 6px; }
        .rbc-overlay::-webkit-scrollbar-track { background: transparent; }
        .rbc-overlay::-webkit-scrollbar-thumb { background: ${isDarkMode ? '#4B5563' : '#D1D5DB'}; border-radius: 10px; }
        .rbc-overlay-header { font-weight: 600; padding: 0.5rem 1rem; border-bottom: 1px solid ${isDarkMode ? '#374151' : '#E5E7EB'}; margin-bottom: 0.25rem; color: ${isDarkMode ? '#E5E7EB' : '#1F2937'}; }
        .rbc-overlay .rbc-event { margin: 4px 12px; padding: 6px 10px; font-size: 0.75rem; border-radius: 6px; color: white; display: block; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

        .rbc-agenda-table { background: ${isDarkMode ? 'rgba(31, 41, 55, 0.5)' : 'rgba(255, 255, 255, 0.6)'}; border-radius: 0.75rem; width: 100% !important; table-layout: auto !important; }
        .rbc-agenda-table th { background: ${isDarkMode ? '#374151' : '#F3F4F6'}; color: ${isDarkMode ? '#E5E7EB' : '#4B5563'}; padding: 10px; text-align: left; font-weight: 600; }
        .rbc-agenda-table td { padding: 8px 10px; border-bottom: 1px solid ${isDarkMode ? '#374151' : '#E5E7EB'} !important; color: ${isDarkMode ? '#E5E7EB' : '#1F2937'} !important; }
        .rbc-agenda-time-cell, .rbc-agenda-date-cell { white-space: nowrap !important; padding-right: 24px !important; width: 1% !important; color: ${isDarkMode ? '#9CA3AF' : '#6B7280'}; }
        .rbc-agenda-view .rbc-agenda-table tbody > tr { background-color: transparent !important; }
        .rbc-agenda-view .rbc-agenda-table tbody > tr:hover { background-color: ${isDarkMode ? 'rgba(99,102,241,0.05)' : 'rgba(168,85,247,0.05)'} !important; }

        @keyframes fadeIn { from { opacity: 0; transform: translateY(-8px); } to { opacity: 1; transform: translateY(0); } }
        .animate-in { animation: fadeIn 0.2s ease-out; }
        .rbc-event { border-radius: 6px !important; transition: all 0.2s ease; cursor: grab !important; margin: 1px 0; z-index: 10; }
        .rbc-event:active { cursor: grabbing !important; }
        .rbc-time-view .rbc-event:hover { transform: scale(1.02); filter: brightness(1.1); z-index: 50 !important; min-width: fit-content; padding-right: 12px !important; }

        @media (max-width: 768px) {
          .rbc-overlay { position: fixed !important; top: 50% !important; left: 50% !important; transform: translate(-50%, -50%) !important; width: 90vw !important; max-width: 350px !important; max-height: 60vh !important; z-index: 9999 !important; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.5) !important; }
          .rbc-time-view { overflow-x: auto !important; -ms-overflow-style: none; scrollbar-width: none; }
          .rbc-time-view::-webkit-scrollbar { display: none; }
          .rbc-time-header, .rbc-time-content { min-width: 500px !important; }
          .rbc-header { font-size: 0.7rem !important; padding: 4px 2px !important; }
        }
        .rbc-addons-dnd-over { background-color: ${isDarkMode ? 'rgba(139, 92, 246, 0.15)' : 'rgba(168, 85, 247, 0.15)'} !important; }
        .rbc-addons-dnd-drag-preview .rbc-event { opacity: 0.6 !important; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.3) !important; transform: scale(1.02); }
        .rbc-addons-dnd-resize-ns-anchor .rbc-addons-dnd-resize-ns-icon { border-top-color: rgba(255, 255, 255, 0.8) !important; border-bottom-color: rgba(255, 255, 255, 0.8) !important; }
        .rbc-time-header.rbc-overflowing { margin-right: 8px !important; border-right: none !important; }
      `}</style>
    </div>
  );
}
