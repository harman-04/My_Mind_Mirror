...
import { Menu, X, Home, Sparkles, Award, User, LogOut, LogIn, UserPlus, Loader } from 'lucide-react';
import ThemeToggle from './components/ThemeToggle';
import AppLogo from './components/AppLogo';
import HeaderApiKeyStatus from './components/HeaderApiKeyStatus';
import { useTheme } from './contexts/ThemeContext';

// Lazy load pages (except HomePage which is small and often first)
    ...
    <div className={`min-h-screen flex flex-col items-center p-2 sm:p-4 bg-gradient-to-br ${bgGradient} text-gray-800 dark:text-gray-200 relative transition-colors duration-500`}>
      ...
      <div className="sm:hidden w-full max-w-4xl flex justify-between items-center py-3 px-4 mb-4 rounded-2xl
        ...
        <div className="flex items-center gap-2">
          <ThemeToggle />
          <button onClick={() => setMobileMenuOpen(true)} className="p-2 rounded-full bg-white/10 hover:bg-white/20 transition">
      ...
      {mobileMenuOpen && (
        <>
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 sm:hidden" onClick={closeMobileMenu} />
          <div className="fixed top-0 right-0 h-full w-80 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl shadow-2xl z-50 sm:hidden flex flex-col p-5">
            <div className="flex justify-between items-center mb-8 pb-2 border-b border-gray-200 dark:border-gray-700">
              ...
              <button onClick={closeMobileMenu} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-800 transition">
                <X size={22} />
              </button>
            </div>
